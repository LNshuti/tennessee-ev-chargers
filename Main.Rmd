...
alt_fuel_stations_davidson <- 
  ...
  mutate(charger_type = ifelse(grepl("BNA|TERM GARAGE", station_name, ignore.case = T), "BNA",
                               charger_type)) %>% 
  ...
  mutate(charger_type = ifelse(grepl("Tesla", station_name, ignore.case = T),
                               "Tesla", charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("Vanderbilt Medical|TCMC B|TriStar Southern",
                                     station_name, ignore.case = T),
                               "Hospital", charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("WHOLEFOODS|TARGET CORP|Publix",
                                     station_name, ignore.case = T),
...
alt_fuel_stations_davidson <-
  alt_fuel_stations_davidson %>% 
  mutate(len_name = str_length(charger_type)) %>%
  filter(str_length(charger_type) < 12)

metro <- 
  alt_fuel_stations_davidson %>% 
  filter(!(charger_type %in% c("Metro", "Premier Parking", "Chargepoint", "Car dealer", "Tesla",
                               "Hotel", "University","Apartment", "BNA", "Grocery store",
                               "Hospital")))
```

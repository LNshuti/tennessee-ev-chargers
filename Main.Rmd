...
alt_fuel_stations_davidson <- 
  ...
  mutate(charger_type = ifelse(grepl("PREMIER|Music City|5TH&BROAD|ENDEAVOR",
                               ...
                               "Paid Parking", charger_type)) %>%
  mutate(charger_type = ifelse(grepl("ENDEAVOR|TVA DEMO", charger_type, ignore.case = T), 
                               "Paid Parking", charger_type)) %>%
  mutate(charger_type = ifelse(grepl("BNA|Terminal Dr", street_address, ignore.case = T), 
                               "Airport", charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("BNA|AIRPORT|MNAA", station_name, ignore.case = T), "Airport",
                               charger_type)) %>%
  mutate(charger_type = ifelse(grepl("BNA|AIRPORT|MNAA", charger_type, ignore.case = T), "Airport",
                               charger_type)) %>%
  ...
  mutate(charger_type = ifelse(grepl("Tesla", station_name, ignore.case = T),
                               "Tesla", charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("Vanderbilt|TCMC|TriStar|NASHVILLE VAMC",
                                     station_name, ignore.case = T),
                               "Hospital", charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("WHOLEFOODS|TARGET|Publix|Mapco",
                                     station_name, ignore.case = T),
                               "Grocery store", charger_type)) %>%
  mutate(charger_type = ifelse(grepl("Willowbrook|BELCOURT", station_name, ignore.case = T),
                                "Retail space", charger_type)) %>%
  mutate(charger_type = ifelse(grepl("State Park|YMCA", station_name, ignore.case = T),
                                "StatePark or YMCA", charger_type)) %>%
...
fix_names <- 
  alt_fuel_stations_davidson %>% 
  filter(!(charger_type %in% c("Metro", "Paid Parking", "Chargepoint", "Car dealer", "Tesla",
                               "Hotel", "University","Apartment", "Airport", "Grocery store",
                               "Hospital", "Apartment complex", "Retail space", "StatePark or YMCA")))
...
pal <- colorFactor(palette = 'Set3', domain = ev_stations_davidson_$charger_type)

davidson_chargers_leaflet <- 
  leaflet(davidson.metro) %>% 
    addTiles() %>% 
    addCircleMarkers(data = ev_stations_davidson_,
                     lat = ~latitude, lng = ~longitude,  color = ~pal(charger_type),
                     popup = paste(ev_stations_davidson_$unique_stations, "<br>",
                                   ev_stations_davidson_$charger_type, "<br>",
                                   ev_stations_davidson_$street_address, "<br>",
                                   ev_stations_davidson_$city, " ",
                                   ev_stations_davidson_$zip)
                     ) %>%
  addLegend(data = ev_stations_davidson_, 

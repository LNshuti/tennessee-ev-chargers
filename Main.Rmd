...
alt_fuel_stations_davidson <- 
  ...
  mutate(charger_type = ifelse(grepl("METRO", station_name, ignore.case = T),
                               "Metro", station_name)) %>% 
  mutate(charger_type = ifelse(grepl("PREMIER|Music City Center|PP 5TH&BROAD|	
ENDEAVOR", station_name, ignore.case = T), 
  ...
  mutate(charger_type = ifelse(grepl("BNA|AIRPORT|MNAA PARK GAR", station_name, ignore.case = T), "Airport",
                               charger_type)) %>% 
  ...
  mutate(charger_type = ifelse(grepl("Marriott|Loews|Duke|Aertson", 
                               ...
                               charger_type)) %>% 
  mutate(charger_type = ifelse(grepl("Shay|Windsor|Rivertop|Apex|oneC1T|Eleven North|Taylor Place",

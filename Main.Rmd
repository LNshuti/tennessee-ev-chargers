...
tn_ev_charger_locations <-
  tn_income_plt + 
  geom_point(data = alt_fuel_stations_tn,
                  aes(x=longitude,y=latitude)) + 
  labs(title = "Tennessee EV charging Infrastructure by County",
...
# Nashville Plot Commuting zones


```{r}
sf_commutingzones_davidson <-
  read_sf("tennessee-ev-chargers/data/commuting-zone/01_commuting-zone-shape-file.shp") %>% 
  st_transform(crs = 4326) %>%
  filter(statefp == "47" & name == "Davidson") 
